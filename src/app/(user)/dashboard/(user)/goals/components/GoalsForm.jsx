...
export default function GoalsForm({ onSave, currentGoals = {}, isUpdateMode = false }) {
    const {
        register,
        handleSubmit,
        setValue,
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 bg-gray-50 dark:bg-neutral-900 p-6 rounded-lg shadow-md">
            {/* Calorie Goal */}
            ...
            <Button type="submit" className="mt-4 w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-lg py-2">
                {isUpdateMode ? "Update Goals" : "Save Goals"}
            </Button>
        </form>
    );
}
