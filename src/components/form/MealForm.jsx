...
const MealForm = ({
  ...
  const onSubmit = async (data) => {
    ...
    if (mode == "create") {
      const response = await axios
        .post("/api/meals/create", updatedData)
        .then((response) => {
      ...
      // Update meal logic
      await axios
        .put(`/api/meals/update/${defaultValues.id}`, updatedData)
        .then((response) => {
          if (response?.data?.success) {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 bg-white dark:bg-neutral-950 shadow-md rounded-md">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="">
        ...
        <div className="mb-4">
          ...
          {errors.steps && (
            <p className="text-red-500">{errors.steps.message}</p>
          )}
        </div>
        {/* Preparation Time */}
        <div className="mb-4">
          ...
          {errors.preparation_time_min && (
            <p className="text-red-500">
              {errors.preparation_time_min.message}
            </p>
          )}
        </div>
